...
def add_extra_rows(array: list[str], row_num: int) -> list[str]:
...
def add_extra_columns(array: list[str], column_num: int) -> list[str]:
    '''Adds an extra comumn after the specified column'''
    
    return_arr = []
    
    for line in array:
        return_arr.append(line[:column_num + 1] + ('.' * NUM_EXPANSIONS) + line[column_num + 1:])
...
for i in reversed(range(len(lines[0]))):
    ...
    if empty:
        lines = add_extra_columns(lines, i)
